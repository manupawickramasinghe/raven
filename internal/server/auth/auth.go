...
func fetchSystemAssertion(baseURL string) (string, error) {
	...
	password := os.Getenv("IDP_SYSTEM_PASSWORD")
...
func fetchAssertion(baseURL, username, password string) string {
	...
	if err := postJSON(baseURL+"/flow/execute", payload, "", &result); err != nil {
		safeUsername := strings.ReplaceAll(strings.ReplaceAll(username, "\n", "\\n"), "\r", "\\r")
		log.Printf("LOGIN: flow execute failed for user %s: %v", safeUsername, err)
		return ""
	}
